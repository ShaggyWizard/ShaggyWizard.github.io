...
export default function PreQuestion({ index }) {
	const { test, setTest, currentLang } = useContext(ToolContext);

	const addAnswer = () => {
		const nextTest = { ...test };
		nextTest.preTest[index].answers.push({});

		setTest(nextTest);
	}
	const handleQuestionChange = (e) => {
		const nextTest = { ...test };
		nextTest.preTest[index].question[currentLang] = e.target.value;
		setTest(nextTest);
	}
	const removeMe = () => {
		const nextTest = { ...test };
		nextTest.preTest.splice(index, 1); 
		setTest(nextTest);
	}
	...
	return (
		<div className="flex flex-col items-stretch p-4 gap-1 bg-blue-200/20">
			...
			<div className="flex gap-2 items-center p-2 ">
				<div className="flex-1 flex flex-col gap-2 items-stretch ">
					<Input
						...
						inputClass="flex-1"
						value={test.preTest[index].question[currentLang] ?? ""}
			...
			{test?.preTest[index]?.answers?.map((_, i) =>
				<PreAnswer key={i} index={i} questionIndex={index} />
			)}
